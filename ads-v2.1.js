(function() {
  ...
  const CONFIG = {
    ...
    BATCH_SIZE: 3, // Process ads in batches
    BATCH_DELAY: 500, // Delay between batches
    ...
    // Preconnect domains for faster loading
    PRECONNECT_DOMAINS: [
  ...
  class AdLogger {
    ...
    log(level, message, data = {}) {
      ...
      if (this.levels[level] >= this.currentLevel) {
        ...
        const prefix = `[AdSense-Optimizer v2.1 ${formattedTime}] [${level}]`;
  ...
  class AdUtils {
    ...
    static isAdLoaded(element) {
      ...
      }).length >= 2; // Require at least two positive checks
    }
    ...
    static removeLoadingPlaceholder(element) {
      const placeholder = element.querySelector('div[style*="background: #f0f0f0"]');
      if (placeholder && placeholder.parentNode) {
        placeholder.remove();
        logger.debug('Loading placeholder removed', { adId: element.getAttribute('data-ad-id') });
      }
    }
  }
    ...
    monitorNetworkChanges() {
      if (navigator.connection) {
        navigator.connection.addEventListener('change', () => {
          ...
          // Adjust retry delay based on network conditions
  ...
  class RetryEngine {
    ...
    shouldRetry(adData, error) {
      ...
      if (!shouldRetry && state.attempts >= maxRetries) {
        this.cooldownPeriods.set(adId, Date.now() + 30000); // 30-second cooldown
    ...
    analyzeError(error) {
      ...
      const severityMap = {
        ...
        'script': 0.7,
        'load': 0.6,
        'blocked': 0.4 // Possible ad blocker
      };
  ...
  class LazyLoadManager {
    ...
    async handleIntersection(entries) {
      for (const entry of entries) {
        if (entry.isIntersecting || entry.intersectionRatio >= CONFIG.INTERSECTION_THRESHOLD) {
          ...
          if (adData && adData.status === 'discovered' && !adData.element.hasAttribute('data-ads-loaded')) {
            logger.debug('Ad entered viewport, loading', { adId: adData.id, intersectionRatio: entry.intersectionRatio });
    ...
    async loadVisibleAds() {
      const visibleAds = Array.from(STATE.adRegistry.values()).filter(ad => {
        ...
        return rect.top < window.innerHeight && rect.bottom > 0 && !ad.element.hasAttribute('data-ads-loaded');
      });
  ...
  class AdSenseController {
    ...
    async initialize() {
      ...
      logger.info('🚀 AdSense Optimizer v2.1 initializing...');
    ...
    async setupDelayedLoading() {
      ...
      const delayTimer = setTimeout(triggerLoad, CONFIG.LOAD_DELAY);
      STATE.timers.add(delayTimer);

      // Trigger 2: On scroll
      const scrollHandler = () => {
        window.removeEventListener('scroll', scrollHandler, { passive: true });
        clearTimeout(delayTimer);
        STATE.timers.delete(delayTimer);
        triggerLoad();
      };
    ...
    async emergencyRecovery() {
      ...
      const failedAds = Array.from(STATE.adRegistry.values()).filter(ad => 
        ad.status === 'failed' && !ad.element.hasAttribute('data-ads-loaded')
      );
  ...
  async function initializeAdSenseOptimizer() {
    try {
      ...
      logger.info('🎉 AdSense Optimizer v2.1 ready and available globally');
      return controller;
    } catch (error) {
  ...
  window.addEventListener('load', () => {
    if (!initPromise) {
      setTimeout(() => tryInitialize('windowLoad'), 500);
    }
  });
