(function() {
  ...
  const CONFIG = {
    ...
    LAZY_LOAD_MARGIN: '400px', // Increased for better page speed
    LOAD_DELAY: 1200, // Optimized for post-load initialization
    MAX_RETRY_ATTEMPTS: 10, // Increased reliability
    RETRY_DELAY: 1200, // Optimized retry timing
    PERFORMANCE_BUDGET: 4500, // Reduced for faster fallback
    HEALTH_CHECK_INTERVAL: 5000, // More frequent checks
    BATCH_SIZE: 3,
    BATCH_DELAY: 250, // Optimized for performance
    CRITICAL_LOAD_TIME: 2000, // Reduced for quicker verification
    TARGET_SUCCESS_RATE: 97, // Higher target success rate
    INTERSECTION_THRESHOLD: 0.15, // Slightly increased for better ad visibility
    PRECONNECT_DOMAINS: [
    ...
    AUTO_ADS_CONFIG: {
      ...
      delay: 1000 // Delay for auto ads initialization
    }
  };
  ...
  class AdLogger {
    ...
    log(level, message, data = {}) {
      ...
      if (this.levels[level] >= this.currentLevel) {
        ...
        const prefix = `[AdSense-Optimizer v3.5 ${formattedTime}] [${level}]`;
  ...
  // Advanced Ad Manager with Enhanced Auto Ads Support
  class AdManager {
    constructor() {
    ...
    async initializeAutoAds() {
      ...
      try {
        await new Promise(resolve => setTimeout(resolve, CONFIG.AUTO_ADS_CONFIG.delay));
        
        (window.adsbygoogle = window.adsbygoogle || []).push({
          google_ad_client: CONFIG.CLIENT_ID,
          ...CONFIG.AUTO_ADS_CONFIG,
          settings: {
            adSense: {
              pageLevelAds: true,
              autoAds: true,
        ...
        logger.info('Auto ads initialized successfully', { config: CONFIG.AUTO_ADS_CONFIG });
      } catch (error) {
        logger.error('Auto ads initialization failed', { error: error.message });
        await new Promise(resolve => setTimeout(resolve, CONFIG.RETRY_DELAY));
        if (this.retryEngine.shouldRetry({ attempts: 0 }, error)) {
          await this.initializeAutoAds();
        }
      }
    }
    ...
    calculatePriority(element) {
      ...
      let priority = 50;
      if (rect.top < window.innerHeight && rect.bottom > 0) priority += 35;
      const area = rect.width * rect.height;
      if (area > 300 * 250) priority += 25;
      if (area > 728 * 90) priority += 20;
      priority += visibility * 25;
      if (rect.top < window.innerHeight / 2) priority += 30;
    ...
    ensureAdAttributes(element) {
      ...
      if (!element.hasAttribute('data-adtest')) {
        element.setAttribute('data-adtest', 'off');
      }
    }
  ...
  class RetryEngine {
    ...
    shouldRetry(adData, error) {
      ...
      const maxRetries = adData.priority > 80 ? CONFIG.MAX_RETRY_ATTEMPTS + 3 : CONFIG.MAX_RETRY_ATTEMPTS;
      const networkInfo = AdUtils.getNetworkInfo();
  ...
  class AdSenseController {
    ...
    async initialize() {
      ...
      logger.info('🚀 AdSense Optimizer v3.5 Flash initializing...');
    ...
    async setupDelayedLoading() {
      ...
      // Enhanced delay mechanism for page speed
      const delayLoad = () => {
      ...
      // Immediate trigger for interactive state
      if (document.readyState === 'interactive' || document.readyState === 'complete') {
        delayLoad();
      } else {
        document.addEventListener('DOMContentLoaded', delayLoad, { once: true });
      }

      // Fallback for slow-loading pages
      setTimeout(() => {
        if (!this.initialized) {
          logger.warn('Fallback timeout triggered');
          triggerLoad();
        }
      }, CONFIG.LOAD_DELAY + 3000);
    }
  ...
  async function initializeAdSenseOptimizer() {
    try {
      ...
      logger.info('🎉 AdSense Optimizer v3.5 Flash ready and available globally');
      return controller;
    } catch (error) {
