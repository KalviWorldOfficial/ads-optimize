(function() {
  ...
  const CONFIG = {
    ...
    LOAD_DELAY: 3000, // Reduced for faster initial load
    MAX_RETRY_ATTEMPTS: 6,
    RETRY_DELAY: 2000,
    PERFORMANCE_BUDGET: 4000,
    HEALTH_CHECK_INTERVAL: 8000, // More frequent checks
    BATCH_SIZE: 3,
    BATCH_DELAY: 500,
  ...
  class AdLogger {
    ...
    log(level, message, data = {}) {
      ...
      if (this.levels[level] >= this.currentLevel) {
        ...
        const prefix = `[AdSense-Optimizer v2.3 ${formattedTime}] [${level}]`;
  ...
  class AdUtils {
    ...
    static preventLayoutShift(element) {
      ...
      element.style.contain = 'content'; // Isolate rendering
      element.style.overflow = 'hidden';
      element.style.zIndex = '0'; // Ensure ad doesn't overlap content
  ...
  class AdSenseController {
    ...
    async initialize() {
      ...
      logger.info('🚀 AdSense Optimizer v2.3 initializing...');
  ...
  async function initializeAdSenseOptimizer() {
    try {
      ...
      logger.info('🎉 AdSense Optimizer v2.3 ready and available globally');
      return controller;
    } catch (error) {
