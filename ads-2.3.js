(function() {
  ...
  const CONFIG = {
    ...
    BATCH_SIZE: 3,
    BATCH_DELAY: 500,
    CRITICAL_LOAD_TIME: 2000, // Increased slightly for reliability
  ...
  class AdLogger {
    ...
    log(level, message, data = {}) {
      ...
      if (this.levels[level] >= this.currentLevel) {
        ...
        const prefix = `[AdSense-Optimizer v2.2 ${formattedTime}] [${level}]`;
  ...
  class AdUtils {
    ...
    static preventLayoutShift(element) {
      ...
      logger.debug('Layout shift prevention applied', { width, height, adId: element.getAttribute('data-ad-id') });
    }

    static createLoadingPlaceholder(element) {
      if (element.hasAttribute('data-placeholder-active')) {
        logger.debug('Placeholder already exists, skipping creation', { adId: element.getAttribute('data-ad-id') });
        return null;
      }
    ...
    static removeLoadingPlaceholder(element) {
      ...
      if (placeholder && placeholder.parentNode) {
        ...
        element.removeAttribute('data-placeholder-active');
        logger.debug('No placeholder found to remove', { adId: element.getAttribute('data-ad-id') });
      }
    }
  }
  ...
  class AdManager {
    ...
    async verifyAdLoad(adData, resolve, reject) {
      let attempts = 0;
      const maxAttempts = 60; // Increased for more thorough verification
    ...
    handleLoadSuccess(adData, loadTime) {
      ...
      AdUtils.removeLoadingPlaceholder(adData.element); // Ensure removal here
      ...
      for (const entry of entries) {
        if (entry.isIntersecting || entry.intersectionRatio >= CONFIG.INTERSECTION_THRESHOLD) {
          ...
          if (adData && 
              ...
              !adData.element.hasAttribute('data-ads-loaded') &&
              !adData.element.hasAttribute('data-ad-processing')) {
            logger.debug('Ad entered viewport, loading', { 
              adId: adData.id, 
  ...
  async function initializeAdSenseOptimizer() {
    try {
      ...
      logger.info('🎉 AdSense Optimizer v2.2 ready and available globally');
      return controller;
    } catch (error) {
